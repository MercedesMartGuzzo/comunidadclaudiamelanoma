...
export default function HeroSection() {
  ...
  const handlePrimaryEnter = () => {
    ...
    gsap.to(primaryTextRef.current, { x: 4, color: '#E3FEF7', duration: 0.4 });
    gsap.to(primaryIconRef.current, { x: 6, stroke: '#E3FEF7', duration: 0.4 });
  };

  const handlePrimaryLeave = () => {
    gsap.to(primaryOverlayRef.current, { x: '-100%', duration: 0.4, ease: 'power2.out' });
    gsap.to(primaryTextRef.current, { x: 0, color: '#003C43', duration: 0.4 });
    gsap.to(primaryIconRef.current, { x: 0, stroke: '#003C43', duration: 0.4 });
  };
