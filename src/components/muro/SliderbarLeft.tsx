        ...
        <div className="flex flex-col gap-4">
            <div className="bg-white rounded-xl p-6 hover:shadow-[0_4px_20px_rgba(0,60,67,0.07)] transition-shadow">
                <div className="flex flex-col items-start mb-5">
                    <div className="relative rounded-full bg-[#E3FEF7] flex items-center justify-center font-inconsolata font-bold text-[#003C43] text-lg mb-3 overflow-hidden w-[56px] h-[56px]">
                        {userAvatar ? (
                            <Image src={userAvatar} alt={userName} fill sizes="56px" className="object-cover" priority />
                        ) : (
                            userName.charAt(0).toUpperCase()
                        )}
                    </div>
                    <p className="font-inconsolata font-bold text-[#003C43] text-sm">{userName}</p>
                    <p className="text-xs text-[#181c1d]/50 font-noto-sans mt-0.5">{userLocation}</p>
                </div>

                <div className="flex flex-col gap-1">
                    {menuItems.map(({ icon: Icon, label, href }) => (
                        <Link key={label} href={href} className="flex items-center gap-2.5 text-sm text-[#181c1d]/70 hover:text-[#003C43] hover:bg-[#f6fafa] rounded-lg px-3 py-2 transition-colors font-noto-sans text-left w-full">
                ...
                <button onClick={handleLogout} className="flex items-center gap-2.5 text-sm text-[#181c1d]/45 hover:text-[#003C43] hover:bg-[#f6fafa] rounded-lg px-3 py-2 transition-colors font-noto-sans text-left w-full">
                ...
                <div className="flex flex-col gap-3">
                    {[
                        ...
                        <div key={label} className="flex items-center justify-between">
                            <span className="text-xs text-[#181c1d]/60 font-noto-sans">{label}</span>
                            <span className="font-inconsolata font-bold text-[#003C43] text-sm">{value}</span>
                        </div>
                    ))}
                </div>
            </div>
