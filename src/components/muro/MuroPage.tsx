...
interface SupabaseCommentJoin {
    ...
    profiles: {
        name: string | null;
        avatar_url: string | null; // <-- CORREGIDO: Agregado para leer la foto desde Supabase
    } | null;
}
...
export default function MuroPage() {
    ...
    // NUEVOS ESTADOS: Para recordar los datos de quien está usando la app actualmente
    ...
    async function fetchFeedData() {
        try {
            ...
            // 1. Obtener datos del usuario logueado actualmente para el Input
            ...
            if (user) {
                ...
                if (profile) {
                    ...
                    setCurrentUserName(profile.name || user.user_metadata?.name || 'Vos');
                }
            }

            // 2. Traer publicaciones
            ...
            // 3. Traer comentarios (CORREGIDO: Ahora pedimos el avatar_url del autor del comentario)
            const { data: commentsData, error: commentsError } = await supabase
                .from('comments')
                .select('id, post_id, user_id, content, created_at, profiles(name, avatar_url)')
            ...
            const mappedComments: CommentCardType[] = ((commentsData || []) as unknown as SupabaseCommentJoin[]).map(c => ({
                ...
                authorAvatar: c.profiles?.avatar_url || null, // <-- CORREGIDO: Mapeamos la foto real desde Supabase
                content: c.content,
    ...
    const handleAddComment = async (postId: string, content: string) => {
        try {
            ...
            const { data: profile } = await supabase
                .from('profiles')
                .select('name, avatar_url') // <-- CORREGIDO: También traemos el avatar para asignarlo instantáneamente
                .eq('id', user.id)
                .maybeSingle();
            ...
            const newComment: CommentCardType = {
                ...
                authorAvatar: profile?.avatar_url || null, // <-- CORREGIDO: Deja de ser null, toma la foto real del usuario al responder
    ...
    return (
        <main className="bg-[#f6fafa] min-h-screen pt-24 pb-10 px-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                    ...
                    <div className="lg:col-span-2 flex flex-col gap-4">
                        ...
                        {loadingFeed ? (
                            ...
                            /* CORREGIDO: Le pasamos los datos del usuario logueado al Feed */
                            <Feed
                                posts={localPosts}
            ...
            <button
                ...
                className="lg:hidden fixed bottom-6 right-6 z-40 w-12 h-12 bg-[#003C43] text-[#E3FEF7] rounded-full shadow-[0_8px_24px_rgba(0,60,67,0.3)] flex items-center justify-center"
            >
