...
interface UserProfile { name: string; location: string; bio: string; }
interface SupabasePost { id: string; user_id: string; content: string; created_at: string; profiles: { name: string; location: string; avatar_url: string | null; } | null; }
interface SupabaseComment { id: string; post_id: string; content: string; created_at: string; profiles: { name: string; avatar_url: string | null; } | null; }
...
export default function UserProfilePage(props: { params: Params }) {
    ...
    const handleAddComment = async (postId: string, content: string): Promise<void> => {
        ...
        const { data: newCommentData } = await supabase.from('comments').insert([{ post_id: postId, user_id: authData.user.id, content: content }]).select('id, post_id, content, created_at, profiles(name, avatar_url)').single();

        if (newCommentData) {
            const raw = newCommentData as unknown as SupabaseComment;
            setComments((prev) => [...prev, { id: raw.id, postId: raw.post_id, content: raw.content, createdAt: raw.created_at, authorName: raw.profiles?.name ?? 'Usuario', authorAvatar: raw.profiles?.avatar_url ?? null }]);
        }
    };
            ...
            <main className="pt-24 pb-10 px-4">
                {isMenuOpen && (
                    <div className="fixed inset-0 z-50 bg-black/50 lg:hidden" onClick={() => setIsMenuOpen(false)}>
                        <div className="fixed inset-y-0 left-0 w-[85%] max-w-sm bg-[#f6fafa] shadow-xl p-6 overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                            <SidebarLeft />
                            <div className="mt-6 pt-6 border-t border-gray-200"><SidebarRight /></div>
                        </div>
                    </div>
                )}
